"""Train MobileNetV3 Small Hybrid (CA + BoT) backbone on Paddy dataset."""
...
from src.models.backbones import MobileNetV3_Small_Hybrid  # noqa: E402
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Train MobileNetV3-Small Hybrid (CA + BoT) on Paddy dataset"
    )
    ...
    parser.add_argument(
        ...
        type=int, 
        default=4, 
        help="Number of attention heads in BoT block"
    )
    parser.add_argument(
        "--reduction", 
        type=int, 
        default=16, 
        help="Reduction ratio for Coordinate Attention block (default: 16)"
    )
    parser.add_argument(
        "--dropout", 
        type=float, 
        default=0.2, 
        help="Dropout probability before classifier (default: 0.2)"
    )
    parser.add_argument(
        "--pretrained", 
        action="store_true", 
    ...
    parser.add_argument(
        "--model-name", 
        type=str, 
        default="MobileNetV3_Small_Hybrid",
...
def print_model_info(model, args):
    ...
    print("\n" + "="*70)
    print("MODEL ARCHITECTURE: MobileNetV3-Small Hybrid (CA + BoT)")
    print("="*70)
    print(f"  Backbone:           MobileNetV3-Small (pretrained={args.pretrained})")
    print(f"  Attention Stack:    CABlock(reduction={args.reduction}) + BoTBlock(heads={args.heads})")
...
def main() -> None:
    ...
    model = MobileNetV3_Small_Hybrid(
