"""Train MobileNetV3 Small CA (Coordinate Attention) backbone on Paddy dataset."""
...
from src.models.backbones import MobileNetV3_Small_CA  # noqa: E402
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Train MobileNetV3-Small CA (Coordinate Attention) on Paddy dataset"
    )
    ...
    parser.add_argument(
        ...
        type=int, 
        default=16, 
        help="Reduction ratio for Coordinate Attention (default: 16)"
    )
    parser.add_argument(
        "--dropout", 
        type=float, 
...
def print_model_info(model, args):
    ...
    print("\n" + "="*70)
    print("MODEL ARCHITECTURE: MobileNetV3-Small CA (Coordinate Attention)")
    print("="*70)
    print(f"  Backbone:           MobileNetV3-Small (pretrained={args.pretrained})")
    print(f"  Attention:          Coordinate Attention Block (reduction={args.reduction})")
    print(f"  Attention Type:     Channel + Spatial attention with coordinate encoding")
