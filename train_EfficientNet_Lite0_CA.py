"""Train EfficientNet-Lite0 với Coordinate Attention trên Paddy dataset."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Train EfficientNet-Lite0 CA trên Paddy dataset"
    )
    ...
    # Tham số khác
    parser.add_argument("--model-name", type=str, default="EfficientNet_Lite0_CA",
...
def main() -> None:
    ...
    # Warmup scheduler: tăng dần LR từ 0 lên target trong warmup_epochs
    warmup_scheduler = LinearLR(
        optimizer, 
        start_factor=0.01,  # Bắt đầu từ 1% của target LR
        end_factor=1.0,     # Kết thúc ở 100% target LR
        total_iters=args.warmup_epochs
    )
    
    # Main scheduler: cosine annealing sau warmup
    main_scheduler = CosineAnnealingLR(
        optimizer, 
        T_max=max(1, t_max - args.warmup_epochs)
    )
    
    # Kết hợp warmup + cosine annealing
    ...
    # Thông báo về model và hyperparameters
    print("✨ EfficientNet-Lite0 + Coordinate Attention")
    print(f"   Params: ~4.7M (nhẹ nhất!)")
    print(f"   Base LR: {args.base_lr:.2e}, Head LR: {args.head_lr:.2e}")
    print(f"   Warmup epochs: {args.warmup_epochs} (ổn định training đầu)")
    print(f"   Weight Decay: {args.weight_decay:.2e}")
    print(f"   Gradient clipping: max_norm={args.gradient_clip}")
    print(f"   CA reduction: {args.reduction}")
    print("   🔧 ĐÃ TỐI ƯU: LR tăng 6x, warmup scheduler, gradient clip chặt hơn\n")
