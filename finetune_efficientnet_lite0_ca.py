"""
Script fine-tune model EfficientNet_Lite0_CA đã train trước trên dataset_0806.
...
from src.models.backbones.efficientnet import EfficientNet_Lite0_CA
...
def load_pretrained_model(
    ...
    # Tạo model architecture EfficientNet_Lite0_CA
    model = EfficientNet_Lite0_CA(
        num_classes=num_classes,
        reduction=16,  # Reduction cho CA attention
        pretrained=False,
...
def main():
    ...
    # Hyperparameters - Tối ưu cho EfficientNet-Lite0
    IMAGE_SIZE = 224
    BATCH_SIZE = 32
    NUM_EPOCHS = 30
    PATIENCE = 8  # Early stopping patience
    BASE_LR = 1e-5  # Learning rate cho backbone (thấp hơn vì EfficientNet nhạy với LR)
    HEAD_LR = 5e-4  # Learning rate cao hơn cho classifier head
    WEIGHT_DECAY = 1e-2  # Weight decay cho regularization
    NUM_WORKERS = 4
    SEED = 42
    ...
    best_epoch = -1
    no_improve_count = 0
    best_model_path = OUTPUT_DIR / "EfficientNet_Lite0_CA_finetuned_best.pt"
    ...
    # Lưu metrics.json
    metrics = {
        "model_name": "EfficientNet_Lite0_CA_finetuned",
    ...
    # Lưu final model 
    final_model_path = OUTPUT_DIR / "EfficientNet_Lite0_CA_finetuned_final.pt"
    torch.save({
        "model": model.state_dict(),
        "metrics": metrics,
        "class_names": class_names
