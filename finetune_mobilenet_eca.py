"""
Script fine-tune model MobileNetV3_Small_ECA đã train trước trên dataset_0806.
...
from src.models.backbones.mobilenet import MobileNetV3_Small_ECA
...
def get_dataloaders(
    ...
    train_transform = transforms.Compose([
        ...
        transforms.RandomRotation(degrees=20),  # Tăng rotation
...
def load_pretrained_model(
    ...
    # Tạo model architecture MobileNetV3_Small_ECA
    model = MobileNetV3_Small_ECA(
        num_classes=num_classes,
        k_size=3,  # Kernel size cho ECA attention
        pretrained=False,
        dropout=0.2  # Dropout cho regularization
    )
...
def main():
    ...
    # Hyperparameters - Tối ưu để giảm overfitting và tăng generalization
    IMAGE_SIZE = 224
    BATCH_SIZE = 32
    NUM_EPOCHS = 30
    PATIENCE = 8  # Early stopping patience
    BASE_LR = 8e-6  # Learning rate cho backbone (thấp để học ổn định)
    HEAD_LR = 3e-4  # Learning rate cho classifier head
    WEIGHT_DECAY = 2e-2  # Weight decay cho regularization
    NUM_WORKERS = 4
    SEED = 42
    ...
    best_epoch = -1
    no_improve_count = 0
    best_model_path = OUTPUT_DIR / "MobileNetV3_Small_ECA_finetuned_best.pt"
    ...
    # Lưu metrics.json
    metrics = {
        "model_name": "MobileNetV3_Small_ECA_finetuned",
    ...
    # Lưu final model 
    final_model_path = OUTPUT_DIR / "MobileNetV3_Small_ECA_finetuned_final.pt"
    torch.save({
        "model": model.state_dict(),
        "metrics": metrics,
        "class_names": class_names
