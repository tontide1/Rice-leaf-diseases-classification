"""Train MobileNetV3 Small ECA (Efficient Channel Attention) backbone on Paddy dataset."""
...
from src.models.backbones import MobileNetV3_Small_ECA  # noqa: E402
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Train MobileNetV3-Small ECA (Efficient Channel Attention) on Paddy dataset"
    )
    ...
    parser.add_argument(
        ...
        type=int, 
        default=3, 
        help="Kernel size for ECA (default: 3, range: 3-9)"
    )
    parser.add_argument(
        "--dropout", 
        type=float, 
...
def print_model_info(model, args):
    ...
    print("\n" + "="*70)
    print("MODEL ARCHITECTURE: MobileNetV3-Small ECA (Efficient Channel Attention)")
    print("="*70)
    print(f"  Backbone:           MobileNetV3-Small (pretrained={args.pretrained})")
    print(f"  Attention:          ECA Block (k_size={args.k_size})")
    print(f"  Attention Type:     Efficient Channel Attention (1D Conv, no dimensionality reduction)")
...
def main() -> None:
    ...
    if args.train_limit:
        print(f"⚠ Training limited to {args.train_limit} samples")
    if args.valid_limit:
        print(f"⚠ Validation limited to {args.valid_limit} samples")
